...
- `ticket_attachments`: id, ticket_id (fk, cascade), comment_id (fk, nullable), url (Vercel Blob), filename, content_type, size_bytes, uploaded_by (fk), created_at.
  **`comment_id` decides WHERE a file renders**: null = attached when the ticket was
  raised, so it belongs in the detail's Attachments section; non-null = posted with a
  reply, so it renders INSIDE that comment's bubble. Both timelines and every detail
  surface (issue, request, recycle-bin preview) split on it. Pooling them into one grid
  made it impossible to tell who attached what, or which reply a file answered.
